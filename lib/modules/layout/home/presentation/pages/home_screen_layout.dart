...
class _HomeScreenLayoutState extends State<HomeScreenLayout> {
  ...
  final Set<String> _preloadedMovies = {};
  ...
  Future<void> _preloadMovieImages(List<MovieEntity> movies) async {
    for (final movie in movies) {
      final movieId = movie.id.toString();

      if (_preloadedMovies.contains(movieId)) {
        continue;
      }

      _preloadedMovies.add(movieId);

      final cover = movie.coverImage;
      final background = movie.backgroundImage;
      ...
      try {
        ...
        if (!mounted) return;
        if (background.isNotEmpty && background.startsWith('http')) {
          await precacheImage(
    ...
    return BlocProvider.value(
      ...
      child: BlocBuilder<HomeCubit, HomeState>(
        builder: (context, state) {
          ...
          final activeIndex =
          _currentIndex.clamp(0, carouselMovies.length - 1);

          final activeMovie = carouselMovies[activeIndex];

          final backgroundImage =
          activeMovie.coverImage.isNotEmpty
              ? activeMovie.coverImage
                        ...
                        children: [
                          Positioned.fill(
                            child: Background(
                              ...
                              movieId: activeMovie.id.toString(),
                            ),
                          ),
                          SafeArea(
                            child: Column(
